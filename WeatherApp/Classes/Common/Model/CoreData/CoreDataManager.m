                                                      ...
                                                      usingBlock:^(NSNotification* note)
         {
             ...
             if (note.object != moc)
                 [moc performBlock:^(){
                     ...
                     NSError* error;
                     NSMutableDictionary* metadata = [[NSPersistentStoreCoordinator metadataForPersistentStoreOfType:NSSQLiteStoreType URL:[self storeUrl] options:nil error:&error] mutableCopy];
                     
                     if (!error && metadata)
                     {
                         [metadata setObject:[NSDate date] forKey:kPersistentStoreLastModified];
                         [NSPersistentStoreCoordinator setMetadata:metadata forPersistentStoreOfType:NSSQLiteStoreType URL:[self storeUrl] options:nil error:&error];
                     }
...
- (NSDate *)lastModifiedDate
{
    ...
    NSDictionary* metadata = [NSPersistentStoreCoordinator metadataForPersistentStoreOfType:NSSQLiteStoreType URL:[self storeUrl] options:nil error:&error];
    
    NSDate* lastModifiedDate;
    
    if (!error)
        lastModifiedDate = metadata[kPersistentStoreLastModified];
